...
router.post("/", multer({
    ...
    storage: diskStorage({
        destination: (_req, _file, cb) => cb(null, path.resolve(mainPath, "..", "tmp", "images")),
    ...
    try {
        ...
        const image: sharp.OutputInfo = await sharp(file.path).resize(512).webp().toFile(path.resolve(mainPath, "images", file.filename.split(".")[0] + ".webp"))
        ...
        const data = {
            ...
            file: file.filename,
            size: file.size,
            url: `https://images.yomumangas.com/images/${file.filename.split(".")[0]}.webp`,
            width: image.width,
            height: image.height
        }
